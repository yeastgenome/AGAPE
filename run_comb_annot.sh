...
ln -s $comb_annot/non_orf.fasta $comb_annot/seq.fastq
...
mv $comb_annot/genes.gff $comb_annot/temp.genes.gff
$SCRIPTS/conv_scf_pos $comb_annot/head.txt $comb_annot/temp.genes.gff > $comb_annot/genes.gff

echo "#" > $comb_annot/$out_name.codex
mkdir -p $comb_annot/more_annot
$SCRIPTS/combined_annot.sh $out_name $comb_annot/more_annot $maker/seq.fasta $comb_annot $comb_annot $snap_dir $SCRIPTS ENSEMBL 80 # results in $comb_annot/more_annot/gff/$out_name.genes.gff
