...
int main(int argc, char **argv) {
	...
	uchar *s;
	FILE *fp;
	char buf[1000], name[1000], gname[1000], dir, *status;
	int i = 0, k = 0, B = 0, E = 0, col = 0;
	bool is_gname_first = false;

	strcpy(buf, "");
	strcpy(name, "");
	strcpy(gname, "");

	if( (argc == 4) && (strcmp(argv[3], "GENE_NAME_FIRST") == 0 ) ) {
		is_gname_first = true;
	}
	else if (argc != 3)
	...
	s = SEQ_CHARS(sf) - 1;
	fp = ckopen(argv[2], "r");
	while ((status = fgets(buf, 1000, fp)) != NULL && !strchr("<>", buf[0]))
		;
	if (status == NULL)
		fatalf("no genes found in %s", argv[2]);
	while (strchr("<>", dir = buf[0])) {
		if (sscanf(buf+1, "%d %d %s %s %*s", &B, &E, gname, name) == 4 ) {
		...
		else if (sscanf(buf+1, "%d %d %s %*s", &B, &E, name) != 3)
			fatalf("ouch: %s", buf);
		else {
			printf(">%s %d-%d\n",
	 		name, B, E);
		...
		N = 0;
		while ((status = fgets(buf, 1000, fp)) != NULL &&
		       !strchr("<>", buf[0]))
