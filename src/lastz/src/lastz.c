...
static const control defaultParams =
	...
	false,0,true,false,					// hspToGapped,searchLimit,searchLimitWarn,searchLimitKeep
	...
	0,-1,								// minMatchCount, maxMismatchCount
...
next_target:
	...
	collectHspsFromBoth = hspsAreAdaptive || (currParams->searchLimit > 0);

	collectHspsSeparately = false;
	if ((collectHspsFromBoth) && (!hspsAreAdaptive))
   ...
   (capinfo*		cap,
	...
	if (info->numPartitions != 0)
		...
		if (partSize == partExpectedOld)
			// $$$ change this; copy the old-format partition list to a new
			// $$$ .. one;  add an 'owner' field to determine whether it should
			// $$$ .. be dealloc'd or not
...
void set_up_hit_processor
   (control*		params,
	...
	filtering =  ((currParams->minIdentity          >  0)
	          ...
	          ||  (currParams->maxMismatchCount     >= 0)
	          ||  (currParams->maxSeparateGapsCount >= 0)
	          ||  (currParams->maxGapColumnsCount   >= 0));
...
int start_one_strand
   (seq*			target,
	...
	//fprintf (stderr, "  total HSPs: %u\n", anchors->len + prevAnchorCount);
...
static u32 report_hsps
   (arg_dont_complain(void* info),
	...
	// $$$йwe should valide that the ends are symmetric about the diagonal
...
static void parse_options_loop
   (int			argc,
	...
	while (argc > 0)
		...
		// --identity=min[..max]
		// $$$ change to recognize --filter=identity:min[..max]
		...
		// --coverage=min[..max]
		// $$$ change to recognize --filter=coverage:min[..max]
		...
		// --continuity=min[..max]
		// $$$ change to recognize --filter=continuity:min[..max]
	...
	if (lzParams->selfCompare)
		...
		if (lzParams->mirrorHSP == -1)
			{ // selfCompare implies mirroring by default, so enable mirroring
			  ...
			  // .. if we later discover that gappedExtend is trtue, we'll
...
static void print_params
   (FILE*		f,
	...
	if (params->tracebackMem != defaultParams.tracebackMem) fprintf (f, "--allocate:traceback=%d\n", params->tracebackMem);
	}
